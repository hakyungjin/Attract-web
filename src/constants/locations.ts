...
export const KOREA_LOCATIONS: LocationData[] = [
  {
    sido: '서울특별시',
    sigungu: [
 ...
 * 전체 지역 목록 (시/도만)
 */
export const SIDO_LIST = KOREA_LOCATIONS.map(loc => loc.sido);

/**
 * 특정 시/도의 시/군/구 목록 반환
 */
export const getSigunguList = (sido: string): string[] => {
  const location = KOREA_LOCATIONS.find(loc => loc.sido === sido);
  return location ? location.sigungu : [];
};

/**
 * 전체 지역을 "시/도 시/군/구" 형태로 반환
 */
export const getAllLocationsFlat = (): string[] => {
  const locations: string[] = [];
  KOREA_LOCATIONS.forEach(loc => {
    loc.sigungu.forEach(sigungu => {
      locations.push(`${loc.sido} ${sigungu}`);
    });
  });
  return locations;
};
